...
The case-file panel includes an optional VirusTotal key box for the person using the viewer. Each user must supply their own key. The key is held only in page memory, is cleared on reload, is never written to a report, and is sent directly to VirusTotal only for hash lookups. Do not enter a key into a shared computer or commit it anywhere.
